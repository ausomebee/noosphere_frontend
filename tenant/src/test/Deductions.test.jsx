...
/**
 * The "Deductions" tab of Payroll settings: one fetch, a permission-gated
 * row menu, the shared PayrollItemModal for add/view/edit and a delete
 * confirmation. The page is a hand-copy of the Income Items tab with its own
 * endpoints, permission keys and nouns — including its own private copy of
 * `formatRateDisplay` — so it is covered separately rather than by proxy.
 *
...
const apiMock = vi.hoisted(() => ({
  ...
  UpdateDeductionsActiveness: vi.fn(),
  CreateDeductions: vi.fn(),
...
import Deductions from "../Pages/Payroll/PayrollSetting/PayrollSettingsSubs/Deductions";
...
beforeEach(() => {
  ...
  apiMock.GetDeductionsByTenantId.mockResolvedValue({ data: ITEMS });
  apiMock.UpdateDeductionsActiveness.mockResolvedValue({});
  apiMock.CreateDeductions.mockResolvedValue({});
  apiMock.UpdateDeductions.mockResolvedValue({});
  apiMock.DeleteDeduction.mockResolvedValue({});
...
describe("loading the list", () => {
  it("asks for the tenant's deductions once the tenant is known", async () => {
    renderPage();
    await loaded();
    expect(apiMock.GetDeductionsByTenantId).toHaveBeenCalledWith({
      tenantId: "tenant-1",
      accessToken: "at",
      refreshToken: "rt",
    });
  ...
  it("fetches nothing while the user has no tenant", async () => {
    ...
    expect(apiMock.GetDeductionsByTenantId).not.toHaveBeenCalled();
  ...
  it("empties the table when the endpoint rejects", async () => {
    apiMock.GetDeductionsByTenantId.mockRejectedValue(new Error("down"));
    renderPage();
  ...
  it("empties the table for a response with no list inside it", async () => {
    apiMock.GetDeductionsByTenantId.mockResolvedValue({ nope: true });
    renderPage();
    await waitFor(() => expect(table.props.loading).toBe(false));
    expect(table.props.data).toEqual([]);
  });
...
describe("the row menu", () => {
  ...
  it("leaves only View to a role that may not edit", async () => {
    renderPage({ permissions: ["add_deductions"] });
    await loaded();
    expect(screen.getByText("View 1")).toBeInTheDocument();
  ...
  it("opens the modal read-only on View", async () => {
    ...
    expect(itemModal.props.isDeduction).toBe(true);
  });
...
describe("adding and editing an item", () => {
  it("hides the add button from a role without the permission", async () => {
    renderPage({ permissions: ["edit_deductions"] });
    await loaded();
    expect(screen.queryByRole("button", { name: "Add New Deduction" })).not.toBeInTheDocument();
  });

  it("opens a blank modal from the add button", async () => {
    renderPage();
    await loaded();
    fireEvent.click(screen.getByRole("button", { name: "Add New Deduction" }));
  ...
  it("creates a flat-rate item and reloads the list", async () => {
    ...
    await waitFor(() =>
      expect(apiMock.CreateDeductions).toHaveBeenCalledWith({
        tenantId: "tenant-1",
        name: "Bonus",
        type: "Flat Rate",
        rate: { rate: 500 },
    ...
    expect(toastMock.showToast).toHaveBeenCalledWith("Deduction created successfully", "success");
    expect(screen.queryByTestId("item-modal")).not.toBeInTheDocument();
    expect(apiMock.GetDeductionsByTenantId.mock.calls.length).toBeGreaterThan(1);
  });
  ...
  it("sends an empty rate for a type it does not recognise", async () => {
    ...
    fireEvent.click(screen.getByText("save-item"));
    await waitFor(() =>
      expect(apiMock.CreateDeductions).toHaveBeenCalledWith(expect.objectContaining({ rate: {} }))
    );
  });
  ...
  it("updates the clicked row and carries its id", async () => {
    ...
    fireEvent.click(screen.getByText("save-item"));
    await waitFor(() =>
      expect(apiMock.UpdateDeductions).toHaveBeenCalledWith(expect.objectContaining({ id: "1" }))
    );
    expect(toastMock.showToast).toHaveBeenCalledWith("Deduction updated successfully", "success");
    expect(apiMock.CreateDeductions).not.toHaveBeenCalled();
  });

  it("reports a failed create and leaves the modal open", async () => {
    apiMock.CreateDeductions.mockRejectedValue(new Error("nope"));
    renderPage();
    await loaded();
    fireEvent.click(screen.getByRole("button", { name: "Add New Deduction" }));
    fireEvent.click(screen.getByText("save-item"));
    await waitFor(() =>
      expect(toastMock.showToast).toHaveBeenCalledWith("Failed to create deduction", "error")
    );
    expect(screen.getByTestId("item-modal")).toBeInTheDocument();
  });

  it("reports a failed update", async () => {
    apiMock.UpdateDeductions.mockRejectedValue(new Error("nope"));
    renderPage();
    await loaded();
    fireEvent.click(screen.getByText("Edit 1"));
    fireEvent.click(screen.getByText("save-item"));
    await waitFor(() =>
      expect(toastMock.showToast).toHaveBeenCalledWith("Failed to update deduction", "error")
    );
  });
...
describe("the status switch", () => {
  it("deactivates an active item and reloads", async () => {
    ...
    await waitFor(() =>
      expect(apiMock.UpdateDeductionsActiveness).toHaveBeenCalledWith({
        id: "1",
        isActive: false,
        accessToken: "at",
        refreshToken: "rt",
  ...
  it("activates an inactive item", async () => {
    ...
    await waitFor(() =>
      expect(toastMock.showToast).toHaveBeenCalledWith(
        "Deduction activated successfully",
        "success"
      )
    );
  });
  ...
  it("reports a failed toggle", async () => {
    apiMock.UpdateDeductionsActiveness.mockRejectedValue(new Error("nope"));
    renderPage();
    await loaded();
    fireEvent.click(screen.getByText("switch 1"));
    await waitFor(() =>
      expect(toastMock.showToast).toHaveBeenCalledWith(
        "Failed to update deduction status",
        "error"
      )
    );
  });
...
describe("deleting an item", () => {
  it("names the row in the confirmation and deletes it", async () => {
    ...
    await waitFor(() =>
      expect(apiMock.DeleteDeduction).toHaveBeenCalledWith({
        id: "1",
        accessToken: "at",
        refreshToken: "rt",
      })
    );
    expect(toastMock.showToast).toHaveBeenCalledWith("Deduction deleted successfully", "success");
  ...
  it("falls back to a generic noun while no row is queued", async () => {
    renderPage();
    await loaded();
    expect(deleteModal.props.message).toContain("this deduction");
  });
  ...
  it("does nothing when confirmed with no row queued", async () => {
    ...
    expect(apiMock.DeleteDeduction).not.toHaveBeenCalled();
  });

  it("surfaces the server's own message on a failed delete", async () => {
    apiMock.DeleteDeduction.mockRejectedValue(new Error("row is referenced"));
    renderPage();
    await loaded();
    fireEvent.click(screen.getByText("Delete 1"));
    fireEvent.click(screen.getByText("confirm-delete"));
  ...
  it("falls back to a generic message when the failure carries none", async () => {
    apiMock.DeleteDeduction.mockRejectedValue({});
    renderPage();
    await loaded();
    fireEvent.click(screen.getByText("Delete 1"));
    fireEvent.click(screen.getByText("confirm-delete"));
    await waitFor(() =>
      expect(toastMock.showToast).toHaveBeenCalledWith("Failed to delete deduction", "error")
    );
  });
